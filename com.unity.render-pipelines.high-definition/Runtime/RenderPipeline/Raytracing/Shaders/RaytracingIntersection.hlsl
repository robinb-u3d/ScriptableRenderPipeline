...
void BuildFragInputsFromIntersection(IntersectionVertice currentVertex, out FragInputs outFragInputs)
{
	...
	outFragInputs.positionRWS = mul(ObjectToWorld(), float4(currentVertex.positionOS, 1.0)).xyz - _WorldSpaceCameraPos;
	...
	outFragInputs.worldToTangent[0] = normalize(mul(currentVertex.tangentOS, worldToObject));
